...
public class TransactionDAO {
	...
	public int add(SecurityTransaction st) {
		String sql = "INSERT INTO TRANSACTIONS (SECURITY_ID, CUSTOMER_ID, BROKER_ID, "
				...
				+ "VALUES (?,?,?,?,?,?,?,?,?,?)";
		return new JdbcTemplate(datasource).update(sql, new Object[] {st.getSecurityId(), st.getCustomerId(), st.getBrokerId(), st.getTransType(), st.getTimedate(), st.getAmount(), st.isStatus(), st.getPurchasePrice(), st.getTotalPrice()});
	}
	
	public Collection<SecurityTransaction> get(Customer customer) {
		String sql = "SELECT TRANASCTION_ID, SECURITY_ID, CUSTOMER_ID, BROKER_ID, TRANS_TYPE, TIMEDATE, AMOUNT, PASS_FAIL, PURCHASE_PRICE, TOTAL_PRICE FROM TRANSACTIONS WHERE TRANSACTION_ID = ?";
	...
	public int update(SecurityTransaction st) {
		...
		return new JdbcTemplate(datasource).update(sql, new Object[] {st.getTransactionId(), st.isStatus()});
	}
	public SecurityTransaction get(int id) {
		String sql = "SELECT TRANSACTION_ID, SECURITY_ID, CUSTOMER_ID, BROKER_ID, TRANS_TYPE, TIMEDATE, AMOUNT, PASS_FAIL, PURCHASE_PRICE, TOTAL_PRICE FROM TRANSACTIONS WHERE TRANSACTION_ID = ?";
	...
	public Collection<SecurityTransaction> getTransactions() {
		String sql = "SELECT TRANSACTION_ID, SECURITY_ID, CUSTOMER_ID, BROKER_ID, TRANS_TYPE, TIMEDATE, AMOUNT, PASS_FAIL, PURCHASE_PRICE, TOTAL_PRICE FROM TRANSACTIONS";
