...
public class TransactionDAO {
	...
	public int add(SecurityTransaction st) {
		String sql = "INSERT INTO TRANSACTIONS (SECURITY_ID, CUSTOMER_ID, BROKER_ID, TRANS_TYPE, TRANS_DATE, TRANS_TIME, AMOUNT, PASS_FAIL, PURCHASE_PRICE, TOTAL_PRICE) VALUES (?,?,?,?,?,?,?,?,?,?)";
		return new JdbcTemplate(datasource).update(sql, new Object[] {st.getSecurityId(), st.getCustomerId(), st.getBrokerId(), st.getTransType(), st.getDate(), st.getTime(), st.getAmount(), st.getStatus(), st.getPurchasePrice(), st.getTotalPrice()});
	}
	...
	public SecurityTransaction get(int id) {
		String sql = "SELECT TRANSACTION_ID, SECURITY_ID, CUSTOMER_ID, BROKER_ID, TRANS_TYPE, TRANS_DATE, TRANS_TIME, AMOUNT, PASS_FAIL, PURCHASE_PRICE, TOTAL_PRICE FROM TRANSACTIONS WHERE TRANSACTION_ID = ?";
		return new JdbcTemplate(datasource).queryForObject(sql, new Object[] {id}, new BeanPropertyRowMapper<SecurityTransaction>());
	}
